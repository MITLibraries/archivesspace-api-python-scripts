...
input')
parser.add_argument('-e', '--endpoint', help='the endpoint for the type of \
records being posted (e.g "resources" or "agents/people"). optional - if not \
...
if args.endpoint:
    ...
    endpoint = input('Enter the endpoint for the type of records being posted \
    (e.g "resources" or "agents/people"): ')
...
f = csv.writer(open('postNew' + date + '.csv', 'w'))
f.writerow(['post'])

records = json.load(open(file))
for i in range(0, len(records)):
    record = json.dumps(records[i])
    post = requests.post(baseURL + '/' + endpoint, headers=headers,
                         data=record).json()
